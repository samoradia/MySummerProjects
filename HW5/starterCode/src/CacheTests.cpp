...
ADD_TEST("Cache operations should be fast, even at a large size.") {
...
ADD_TEST("Can put items after other member functions are called"){
    Cache<int> cache(3);
    ...
    //calling put after running containsKey and get should work
    cache.put("E", 5);
    expect(!cache.containsKey("B"));
    expectError(cache.get("B"));

}
ADD_TEST("Keys that were never entered into cache are not found by ContainsKey"){
    Cache<int> cache(3);
