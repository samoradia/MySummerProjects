...
private:
    ...
    Node* root;
    /**
     * This helper function will recursively call itself from the constructor to create a balanced
     * BST, given the middle index of the Line vector, which should be the middle
     */
    Node* LineManagerHelper(const Vector<Line *>& lines, int start, int end);

    /**
     * This helper function will recursively search through the BST to find all values that fit in the range. If it is in the
     * range, it will be added to a vector that is an outparameter
     *
     */
    void RangeHelper( double min,  double max,  Node* root,  Vector<Line *>& lines) const;

    /**
     * This helper function will search the BST for the node that has the line pointer that takes up the range that
     * includes the inputted y value
     *
     */
    Line* LineAtHelper(double y, Node* root) const;

    /**
     * This helper function will recursively go through the tree, deleting the line pointer associated
     * with the node, then the node itself.
     */
