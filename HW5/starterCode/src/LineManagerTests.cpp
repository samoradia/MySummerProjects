...
ADD_TEST("Does not crash when given an empty vector of lines"){
    Vector<Line *> lines;

    //initialize LineManager with an empty vector
    LineManager manager(lines);

    //empty vector should mean no height, and no lines
    expect(equal(manager.contentHeight(), 0));
    expect(manager.lineAt(10) == nullptr);
    //arbitrary min and max values, any pairing of numbers should produce the same outcome
    expect(manager.linesInRange(10,50).isEmpty());
    expect(manager.linesInRange(-86,768).isEmpty());
}
ADD_TEST("Clicking on the edge of the line still returns it"){
    ...
    // hit at upper boundary
    expect(manager.lineAt(0) == lines[0]);
    //hit the lower boundary
    expect(manager.lineAt(50) == lines[0]);
}
